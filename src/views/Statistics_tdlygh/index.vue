...
export default {
    ...
    data() {
        return {
            ...
            barData:{
                    dimensions: ['product', '城乡建设用地', '有条件建设区', '基本农田', '其它非建设用地'],
                    source: [{product:'所选累计',城乡建设用地:0,有条件建设区:0,基本农田:0,其它非建设用地:0}]
            },
            result:{},
            pieVis:false,
            legendData:[],
    ...
    methods: {
        ...
        initLedger(){
            for(let e in this.result){
                ...
                ele.per_1   = _res.data[1].value;
                ele.area_2  = _res.data[2].value;
                ele.per_2   = _res.data[3].value;
                ele.area_3  = _res.data[4].value;
                ele.per_3   = _res.data[5].value;
                ele.area_4  = _res.data[6].value;
                ele.per_4   = _res.data[7].value;
        ...
        //处理echarts用的数据
        echartsData(){
            let res = this.result,pData0=0,pData1=0,pData2=0,pData3=0;
            if(this.currentAreasArr == '全市'){
                pData0 = res[11].data[0].value;
                pData1 = res[11].data[2].value;
                pData2 = res[11].data[4].value;
                pData3 = res[11].data[6].value;
            }else{
                for(let i in res){
                     
                    if(this.currentAreasArr.includes(res[i].xzq)){
                        pData0 = res[i].data[0].value;
                        pData1 = res[i].data[2].value;
                        pData2 = res[i].data[4].value;
                        pData3 = res[i].data[6].value;
                    }
                }
            }
        ...
        initBarcharts(){
            ...
            //初始化要展示的数据
            var a = [];
            self.barData.source = [{product:'所选累计',城乡建设用地:0,有条件建设区:0,基本农田:0,其它非建设用地:0}];
            ...
            a.forEach(ele=>{
                    ...
                    self.barData.source[0].城乡建设用地 += res.find(eve=>{return eve.xzq == ele}).data[0].value;
                    self.barData.source[0].有条件建设区 += res.find(eve=>{return eve.xzq == ele}).data[2].value;
                    self.barData.source[0].基本农田   += res.find(eve=>{return eve.xzq == ele}).data[4].value;
                    self.barData.source[0].其它非建设用地 += res.find(eve=>{return eve.xzq == ele}).data[6].value;
            })
             self.barData.source[0].城乡建设用地 = (self.barData.source[0].城乡建设用地).toFixed(4);
             self.barData.source[0].有条件建设区 = (self.barData.source[0].有条件建设区).toFixed(4);
             self.barData.source[0].基本农田 = (self.barData.source[0].基本农田).toFixed(4);
             self.barData.source[0].其它非建设用地 = (self.barData.source[0].其它非建设用地).toFixed(4);
        ...
        initPiecharts(){
            ...
            // 绘制图表
            let option = {
                color:["rgb(225,225,225)","rgb(197,0,255)","rgb(255,255,102)","rgb(129,219,120)"],
                title: {
                    text:'',
                    x: 'center',
                },
...
.Stat_table /deep/ .el-table__header-wrapper{
.el-table__header{
