...
export default {
    ...
    data() {
        return {
            ...
            barData:{
                    dimensions: ['product', '一级管控区', '二级管控区'],
                    source: [
                    ]
            },
            result:{},
    ...
    methods: {
        ...
        initLedger(){
            ...
            for(let e in this.result){
                let ele ={}, _res = this.result[e][0];
                ele.region = _res.区属;
                ele.num_1  = _res.NUM_YJ;
                ele.area_1  = (_res.AREA_YJ/10000).toFixed(4);
                ele.per_1    = _res.ZB_YJ;
                ele.num_2  = _res.NUM_EJ;
                ele.area_2  = (_res.AREA_EJ/10000).toFixed(4);
                ele.per_2   = _res.ZB_EJ;
                ele.num_total  = _res.个数小计;
                ele.area_total  = (_res.面积小计/10000).toFixed(4);
                ele.per_total   = _res.ZB_XJ;
        ...
        echartsData(){
            let res = this.result , pData0 = 0, pData1 = 0;
                for(let i in res){
                    ...
                    pData0 += (res[i][0].AREA_YJ/10000)
                    pData1 += (res[i][0].AREA_EJ/10000)
                    }else{
                       if(this.currentAreasArr.includes(i)){
                            pData0 += (res[i][0].AREA_YJ/10000)
                            pData1 += (res[i][0].AREA_EJ/10000)
                       }
                    }
        ...
        initBarcharts(){
            ...
            a.forEach(ele=>{
                    ...
                    let e = {};
                    e.product = ele;
                    e.一级管控区 = (res[ele][0].AREA_YJ/10000).toFixed(4);
                    e.二级管控区 = (res[ele][0].AREA_EJ/10000).toFixed(4);
        ...
        initPiecharts(){
            ...
            // 绘制图表
            let option = {
                color:['rgb(255,0,0)','rgb(245,125,125)'],
                title: {
                    text:'',
                    x: 'center',
                },
...
.Stat_table /deep/ .el-table--scrollable-x .el-table__body-wrapper{
    overflow-y: scroll;
    height:192px;
}
#chart{
